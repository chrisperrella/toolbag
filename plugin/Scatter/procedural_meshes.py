...
def calculate_normals(vertices: list, triangles: list) -> list:
    ...
    for i in range(0, len(triangles), 3):
        ...
        face_normal = [edge1[1] * edge2[2] - edge1[2] * edge2[1], edge1[2] * edge2[0] - edge1[0] * edge2[2], edge1[0] * edge2[1] - edge1[1] * edge2[0]]
        for idx in [i0, i1, i2]:
...
def create_terrain(
    width: float = 2.0, height: float = 2.0, subdivisions_x: int = 50, subdivisions_y: int = 50, noise_scale: float = 0.3, noise_frequency: float = 4.0
) -> mset.MeshObject:
    vertices = []
    triangles = []
    uvs = []
