...
def fbm_noise(u: float, v: float, octaves: int = 4, frequency: float = 4.0, amplitude: float = 1.0, lacunarity: float = 2.0, gain: float = 0.5) -> float:
    value = 0.0
    freq = frequency
    amp = amplitude
...
def ring(u: float, v: float, center: Tuple[float, float] = (0.5, 0.5), inner_radius: float = 0.2, outer_radius: float = 0.4) -> float:
    cx, cy = center
    du, dv = u - cx, v - cy
    distance = math.sqrt(du * du + dv * dv)
...
def spiral(u: float, v: float, center: Tuple[float, float] = (0.5, 0.5), turns: float = 4.0, thickness: float = 0.1) -> float:
    cx, cy = center
    du, dv = u - cx, v - cy
    distance = math.sqrt(du * du + dv * dv)
    if distance < 1e-6:
